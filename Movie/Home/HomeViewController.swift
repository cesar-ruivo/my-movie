...
extension HomeViewController: HomeHeaderDelegateProtocol {
    func buttonFavoriteTapped() {
        presenter.didSelectFavoriteIcone()
    }
...
extension HomeViewController: HomePresenterToViewProtocol {
    ...
    func updateSpecificIcon(at indexPaths: [IndexPath], isFavorite: Bool) {
        for indexPath in indexPaths {
            if let cell = collectionView.cellForItem(at: indexPath) as? BannerMovieGridCell {
                cell.updateFavoriteState(isFavorite: isFavorite)
            }
            else if let cell = collectionView.cellForItem(at: indexPath) as? MovieGridCell {
                cell.updateFavoriteState(isFavorite: isFavorite)
            }
        }
    }
