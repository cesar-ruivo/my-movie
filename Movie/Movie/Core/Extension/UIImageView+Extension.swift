...
extension UIImageView {
    ...
    func loadImage(from urlString: String, placeholder: UIImage? = nil) {
        ...
        // Define um placeholder enquanto baixa a imagem
        self.image = placeholder
        
        // Guarda a última URL requisitada para evitar imagens trocadas
        let lastURLUsed = urlString
        
        // Se existir no cache, usa diretamente
        if let cachedImage: UIImage = imageCache.object(forKey: NSString(string: urlString)) {
            DispatchQueue.main.async {
                // Confere se ainda é a URL certa antes de setar
        ...
        URLSession.shared.dataTask(with: url) { [weak self] data, _, error in
            ...
            // Atualiza imagem apenas se for a URL correta
            DispatchQueue.main.async {
                if lastURLUsed == urlString {
